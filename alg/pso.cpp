...
int D, TF;
//nebo 50, hlavně ať je to dělitelné 5000 - experimentuj a dívej se jak to vychází
...
void initPopulation(std::vector<Particle> &population, std::vector<double> &leadingPosG, double gCost)
{
    for (int i = 0; i < popSize; i++)
    {
        ...
        //Initialize the particle's position with a uniformly distriBOUNDARY_UPted random vector xi ~ U(BOUNDARY_LOW, BOUNDARY_UPp)
        //TODO initial velocity 0?
        //Initialize the particle's velocity: vi ~ U(-|BOUNDARY_UPp-BOUNDARY_LOW|, |BOUNDARY_UPp-BOUNDARY_LOW|)
